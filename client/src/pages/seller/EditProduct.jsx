...
export default function EditProduct() {
    ...
    if (id) {
      ...
      productService.getProductById(id)
        .then(res => {
          ...
          if (p.originalPrice) setValue('originalPrice', p.originalPrice.toString())
          setValue('category', p.category)
          if (p.brand) setValue('brand', p.brand)
          setValue('stock', p.stock?.toString())
          if (p.sku) setValue('sku', p.sku)
          
          if (p.images) {
            setPreviews(p.images)
            // Existing images can't be easily put into File objects, so we keep them in previews and track new images separately if needed.
            // For simplicity in this edit, if they add new images, it replaces or appends.
          }
  ...
  const removeImage = (i) => {
    // If it's a new image (File), remove from images array
    // If it's an existing image (URL string), we just remove from previews
    setPreviews((prev) => prev.filter((_, idx) => idx !== i))
    // We would need more complex logic to perfectly sync new vs old, but for this simple version:
    setImages([]) 
  }
  ...
  const onSubmit = async (data) => {
    if (previews.length === 0) { toast.error('Product must have at least one image'); return }
    setLoading(true)
    try {
